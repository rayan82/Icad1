...
class Animal extends Model
{
    ...
    public function historiqueAnimal($id)
    {
        ...
        $db = \Config\Database::connect();
        $result = $db->query("SELECT log_animal.ID_ICAD, log_animal.DATE_ACTION, log_animal.TYPE_ACTION, log_animal.NOM_ANIMAL, log_animal.DATE_NAISSANCE_ANIMAL, log_animal.INFO_ANIMAL, log_animal.RACE_ANIMAL, proprietaire.NOM_PROPRIO AS NOM_PROPRIETAIRE, sexe_animal.NOM_SEXE AS SEXE_ANIMAL, espece_animal.NOM_ESPECE AS ESPECE_ANIMAL, log_animal.IS_PERDU_ANIMAL ,log_animal.VILLE_ANIMAL , log_animal.DATE_PERTE_ANIMAL,log_animal.CIRCONSTANCE_PERTE_ANIMAL FROM log_animal LEFT JOIN proprietaire ON log_animal.ID_PROPRIO = proprietaire.ID_PROPRIO JOIN sexe_animal ON log_animal.SEXE_ANIMAL = sexe_animal.ID_SEXE JOIN espece_animal ON log_animal.ESPECE_ANIMAL = espece_animal.ID_ESPECE WHERE ID_ICAD = " . $id . " ORDER BY DATE_ACTION");
        return $result->getResultArray();
    }
